...
const Incidencias = () => {
  ...
  return (
    ...
    <div className="flex justify-center  mt-11">
        ...
        <select
    className="block py-2 px-0 w-full text-lg text-center text-gray-500 bg-transparent border-0 border-indigo-700 border-b-4 appearance-none focus:outline-none focus:ring-0 focus:border-indigo-600 peer"
>
    <option className='w-full' value="">--Selecciona el tipo de equipo que quieres reportar--</option>
    {tipoEquipos && tipoEquipos.length > 0 && tipoEquipos.map((tipoEquipo) => (
        <option key={tipoEquipo.id} value={tipoEquipo.id}>{tipoEquipo.nombre}</option>
            ...
            <textarea className="border-4 border-opacity-50 border-indigo-400 shadow-lg" id="" placeholder="Brinda detalles del reporte" cols="30" rows="5"></textarea>
